...
- [7.15 - Live Event Signaling](implementation.md#livestream)
...
| `recorded` | integer | An enumeration indicating how the content was recorded:<br><br>0 = not pre-recorded<br>1 = yes, pre-recorded<br><br>See [Section 7.15 of Implementation Guidance](implementation.md#livestream) for additional detail.  |
| `capturedlive` | integer | An enumeration indicating if the event is happening in real time while it is being watched:<br><br>0 = not captured live<br>1 = yes, captured live<br><br>See [Section 7.15 of Implementation Guidance](implementation.md#livestream) for additional detail. |
| `firstbroadcast`| integer | An enumeration indicating if the stream is the first broadcast of the content:<br><br>0 = no, not the first broadcast of the program<br>1 = yes, first time the program is being broadcast<br><br>See [Section 7.15 of Implementation Guidance](implementation.md#livestream) for additional detail. |
