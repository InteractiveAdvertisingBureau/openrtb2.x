...
- [7.10 - Updated Video Signals](https://github.com/InteractiveAdvertisingBureau/openrtb2.x/blob/develop/implementation.md#710---updated-in-stream-values)
...
<table>
  ...
  <tr>
    ...
    <td>Currency specified using ISO-4217 alpha codes. This may be different from bid currency returned by bidder if this is allowed by the exchange. This currency applies to all floors specified in the `Banner`, `Video`, `Audio`, `Native`, and `Format` objects.</td>
  </tr>
  <tr>
...
4 = iframe.<br></td>
  ...
  <tr>
    <td><code>bidfloor</code></td>
    <td>float; default 0</td>
    <td>Minimum bid for this impression expressed in CPM, if the buyer bids with a banner creative. This is most relevant for establishing pricing in multi-format requests (e.g. banner or video creatives both eligible for a given impression opportunity, at a floor of $1 and $2, respectively). See the `Format` object regarding per-size floors. If `bidfloor` is not present in the `Banner` object, then `bidfloor` at the `Imp` level will serve as the default floor.</td>
  </tr>
  <tr>
    <td><code>ext</code></td>
    <td>object</td>
...
<table>
  ...
  <tr>
    <td><code>bidfloor</code></td>
    <td>float; default 0</td>
    <td>Minimum bid for this impression expressed in CPM if the buyer bids with a video creative, regardless of duration. This is most relevant for establishing pricing in multi-format requests (e.g. banner or video creatives both eligible for a given impression opportunity, at a floor of $1 and $2, respectively). If `bidfloor` is not present in the `Video` object, then `bidfloor` at the `Imp` level will serve as the default floor.</td>
  </tr>
  <tr>
    <td><code>durfloors</code></td>
    <td>object array</td>
    <td>An array of `Durfloor` objects (Section 3.2.35) indicating the floor prices for video creatives of various durations that the buyer may bid with.</td>
  </tr>
  <tr>
    <td><code>ext</code></td>
    <td>object</td>
...
<table>
  ...
  <tr>
    <td><code>bidfloor</code></td>
    <td>float; default 0</td>
    <td>Minimum bid for this impression expressed in CPM if the buyer bids with an audio creative, regardless of duration. If `bidfloor` is not present in the `Audio` object, then `bidfloor` at the `Imp` level will serve as the default floor.</td>
  </tr>
  <tr>
    <td><code>durfloors</code></td>
    <td>object array</td>
    <td>An array of `Durfloor` objects (Section 3.2.35) indicating the floor prices for audio creatives of various durations that the buyer may bid with.</td>
  </tr>
  <tr>
    <td><code>ext</code></td>
    <td>object</td>
...
<table>
  ...
  <tr>
    <td><code>bidfloor</code></td>
    <td>float; default 0</td>
    <td>Minimum bid for this impression expressed in CPM if the buyer bids with a native creative, regardless of duration.</td>
  </tr>
  </tr>
    <td><code>ext</code></td>
    <td>object</td>
...
<table>
  ...
  <tr>
    <td><code>bidfloor</code></td>
    <td>float; default 0</td>
    <td>Minimum bid for this impression opportunity expressed in CPM, if the buyer bids with a creative of the respective size (i.e. 728x90 at a floor of $1.50 or 320x50 at a floor of $1.40). For any sizes in the `Format` array that do not have a `bidfloor`, the `bidfloor` in the `Banner` object will serve as the default floor. If `bidfloor` is not present in `Banner`, then `bidfloor` at the `Imp` level will serve as the default floor.</td>
  </tr>
  <tr>
...
<table>
  ...
  <tr>
   ...
   <tr>
    <td><code>bannerfloors</code></td>
    <td>object array</td>  
    <td>Container for floor price by banner size information, to be used if a given deal is eligible for banner demand. An array of Format objects (see Section 3.2.12).</td>
   </tr>
   <tr>
    <td><code>videofloors</code></td>
    <td>object array</td>  
    <td>Container for floor price by duration information, to be used if a given deal is eligible for video demand. An array of DurFloors objects (see Section 3.2.35).</td>
   </tr>
   <tr>
    <td><code>audiofloors</code></td>
    <td>object array</td>  
    <td>Container for floor price by duration information, to be used if a given deal is eligible for audio demand. An array of DurFloors objects (see Section 3.2.35).</td>
   </tr>
   <tr>
    <td><code>ext</code></td>
    <td>object</td>
...
Allows publishers to specify prices for audio or video creatives of different durations. There are no explicit constraints on the defined ranges, nor guarantees that they don't overlap. This is between the seller and buyer to sort out. In simple terms, this object is used by sellers to provide pricing information to buyers by duration. For example: 1-15 seconds at a floor of $5; 16-30 seconds at a floor of $10, > 31 seconds at a floor of $20.
...
<table>
  ...
  <tr>
    <td><code>bidfloor</code></td>
    <td>float; default 0</td>
    <td>Minimum bid for a given impression opportunity, if bidding with a creative in this duration range, expressed in CPM. For any creatives whose durations are outside of the defined min/max, the `bidfloor` in the corresponding `Video` or `Audio` object will serve as the default floor. If `bidfloor` is not present in `Video` or `Audio`, then `bidfloor` at the `Imp` level will serve as the default floor.</td>
  </tr>
  <tr>
    <td><code>ext</code></td>
    <td>object</td>
