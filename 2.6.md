...
<table>
  ...
  <tr>
    ...
    <td>string array</td>
    <td>dThis parameter allows an app publisher to allow impressions only from specific app advertisers (allow list) based on app IDs, for example to limit sponsored placements to within a group of companies, or to provide a first preference.</td>
  </tr>
  <tr>
    <td><code>bapp</code></td>
    <td>string array</td>
    <td>This parameter allows an app publisher to block impressions from specific app advertisers (block list) based on app IDs, for example to ensure control, relevancy and user experience (eg. blocking competitors).</td>
  </tr>
  <tr>
    <td><code>ext</code></td>
    <td>object</td>
